const bullets = {
  ...
  kill() {
    this.list.forEach((bullet) => {
      if (
        bullet.timeSpawned <
        simulation.time - this.duration * bullet.origin.bulletDuration
      ) {
  ...
  killExplosions() {
    this.explosions.forEach((xpl) => {
      if (xpl.time + this.explosionDuration < simulation.time)
        this.explosions.splice(this.explosions.indexOf(xpl), 1)
    })
  },
  muzzleFlash() {
    draw.beginPath()
    ...
    this.list.forEach((bullet) => {
      if (bullet.isHoming) {
        mobs.list.forEach((mob) => {
          if (
            utils.distance(bullet.pos.x, bullet.pos.y, mob.pos.x, mob.pos.y) <
            mob.size * 12
          ) {
            bullet.angle = Math.atan2(
              mob.pos.y - bullet.pos.y,
              mob.pos.x - bullet.pos.x,
            )
          } else
            bullet.angle = Math.atan2(
              input.cursor.y - bullet.pos.y,
              input.cursor.x - bullet.pos.x,
            )
        })
      }
